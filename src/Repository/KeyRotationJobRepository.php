...
use BlackCat\Database\Packages\KeyRotationJobs\Dto\KeyRotationJobDto as Dto;
use BlackCat\Database\Packages\KeyRotationJobs\Mapper\KeyRotationJobDtoMapper as RowMapper;
...
final class KeyRotationJobRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KeyRotationJobs\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KeyRotationJobs\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyRotationJobs\Dto\KeyRotationJobDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KeyRotationJobs\Dto\KeyRotationJobDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KeyRotationJobs\Mapper\KeyRotationJobDtoMapper::fromRow($row);
    }
