 ...
 */
final class KeyRotationJobsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private KeyRotationJobRepository $keyRotationJobRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'key_rotation_jobs' => $this->keyRotationJobRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\KeyRotationJobs\ModuleException("Record {$idStr} not found for locking.");
            }
