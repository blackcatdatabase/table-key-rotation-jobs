...
final class KeyRotationJobsModule implements ModuleInterface
{
    public function name(): string { return 'table-key_rotation_jobs'; }
    public function table(): string { return 'key_rotation_jobs'; }
...
SQL;
    ...
    public function status(Database $db, SqlDialect $d): array
    {
        ...
        $expectedIdx = [ 'idx_key_rotation_jobs_basename_sched' ];
        if ($d->isMysql()) {
        ...
        $expectedFk  = [ 'fk_key_rotation_jobs_user' ];
    ...
    public function info(): array
    {
        return [
            ...
            'indexes'     => [ 'idx_key_rotation_jobs_basename_sched' ],
            'foreignKeys' => [ 'fk_key_rotation_jobs_user' ],
        ];
    }
}
